...
function EmojiPickerDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed bottom-0 left-0 right-0 bg-base-100 shadow-xl rounded-t-2xl transition-all duration-200
          ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}
        style={{ zIndex: 9999, height: '60vh' }}
      >
        <div className="p-4 pb-[env(safe-area-inset-bottom)]">
          ...
          {/* Emoji Grid */}
          <div className="grid grid-cols-6 gap-3 max-h-[55vh] overflow-y-auto p-1">
            {filtered.map((item) => (
              <button
                key={item.id}
